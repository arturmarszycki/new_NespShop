...
class Category extends Component {
    render() {
        const {list, lang, updateCart, shop, showDetails} = this.props;
        return (
            <div className="single-category" id={`category_${list[0].id_shop_category}`}>
                ...
                <ul className="capsule-list">
                    {list.map(item => {
                        return (
                            <Capsule key={item.id_shop_product} lang={lang} item={item} updateCart={updateCart} shop={shop} showDetails={showDetails} />
                        )
                    })}
                </ul>
            </div>
